                        ...
                        tabItems(
                          tabItem(tabName = "elso" ,
                           ...
                           tabItem(tabName = "harmadik",
                                   fluidRow(column(9,
                                                  tags$style(HTML("#hipergeo_box1 .inner {background:   	#006666}")),
                                                  tags$style(HTML("#hipergeo_box2 .inner {background:   	#009999}")),
                                                  tags$style(HTML("#hipergeo_box3 .inner {background:   	#00CCCC}")),
                                                  tags$head(tags$style(HTML('.skin-green .main-header .logo {background-color: #0f7864}
                                                  .skin-green .main-header .logo:hover {background-color: #0f7864}
                                                  .skin-green .main-header .navbar {background-color: #18bc9c}
                                                  .skin-green .main-header .navbar .sidebar-toggle:hover{
                                           ...
                                           column(3,
                                                  tabBox(width = "110%",side="right",
                                                         tabPanel("Várható érték",uiOutput('hipergeo_ex2')),
                                                         tabPanel("Szórás",uiOutput('hipergeo_ex3')),
                                                         tabPanel("Hipergeometriai eloszlás", uiOutput('hipergeo_ex1')))
                                                  )),
                                  ...
                                  fluidRow(
                                    column(2,
                                           box(title = "Diagram" ,width = NULL,solidHeader = TRUE,
                                               collapsible = TRUE,collapsed=TRUE, status = "success",
                                               selectInput("hsulyplot_tipus","Súlyfüggvény ábra típusának megváltoztatása:",
                                                           c("Oszlopdiagram"="hipergeo_plot_1", "Pontdiagram"="hipergeo_plot_2" )),
                                               selectInput("heloszlplot_tipus","Eloszlásfüggvény ábra típusának megváltoztatása:",
                                                           c( "Lépcsős diagram"="hipergeo_eloszlplot_2", "Vonaldiagram"="hipergeo_eloszlplot_1" ))),
                                           ...
                                           gradientBox(width = 12,
                                             ...
                                             collapsible = FALSE,
                                             footer = list(
                                               sliderInput("hipergeo_n", "Adja meg az \\(n\\) értékét:",
                                                                 15, min = 0, max = 100, step = 1),
                                               sliderInput("hipergeo_m", "Adja meg az \\(m\\) értékét:", 
                                                             15, min = 0, max = 100, step = 1),
                                               uiOutput("hipergeo_slider_1",inline = T)
                                    ...
                                    column(2,
                                           ...
                                           box(title = "Adattábla", width = NULL, solidHeader = TRUE, status = "success",
                                               collapsible = TRUE,collapsed=TRUE, DT::dataTableOutput("hipergeo_tabla")),
                                           ...
                                           gradientBox(width = 12,
                                             ...
                                             collapsible = FALSE,
                                             footer = list(withMathJax(
                                               selectInput("hipergeo_tipus","Adja meg a valószínűségi típust:",
                                                            c("\\(P(X=x)\\)"="hipergeo_egyenlo", "\\(P(X<x)\\)"="hipergeo_kisebb",
                                                              "\\(P(X \\leq x)\\)"="hipergeo_ekisebb","\\(P(X>x)\\)" = "hipergeo_nagyobb",
                                                              "\\(P(X \\geq x)\\)" = "hipergeo_enagyobb",
                                                              "\\(P(\\ X_1 \\leq x  \\leq \\ X_2)\\)"="hipergeo_intervallum" ))
                                               ),
                                               
                                           
                                               sliderInput("hipergeo_x", "Adja meg az \\(x\\) értékét:",
                                                       min = 0, max = 100, value = c(5), step = 1),
                                           
                                               sliderInput("hipergeo_x_intervallum","(Intervallum esetén):",
                                                       min = 0, max = 100, value = c(0,5), step = 1)
...
output$binom_slider_1 <- renderUI({
  sliderInput("binom_x", "Adja meg az \\(x\\) értékét:", 
                    min = 1, max(input$binom_n), value = c(5), step = 0.5)})  
...
output$poisson_plot <- renderPlotly({
  ...
  eloszlasfvg <- if (input$peloszlplot_tipus == "poisson_eloszlplot_1")
    plot_ly(y=ppois(poisson_lambda_range(), input$poisson_lambda),
            ...
            name = "(x,y)",
            mode = 'lines',line = list(color = '#009999'))
  else if (input$peloszlplot_tipus == "poisson_eloszlplot_2")
    plot_ly(y=ppois(poisson_lambda_range(), input$poisson_lambda),
            x=poisson_lambda_range(),
            type = "violin",
            name = "(x,y)",
...
output$poisson_ex1 <- renderUI({
    withMathJax(helpText('$$ X\\sim\\text{Poi}(\\lambda) $$
...
output$hipergeo_plot <- renderPlotly({
  sulyfvg <- if (input$hsulyplot_tipus == "hipergeo_plot_1")
    plot_ly(y=dhyper(hipergeo_n_range(),input$hipergeo_m,input$hipergeo_n,input$hipergeo_k),
            x=hipergeo_n_range(),
            type = "bar",
            name = "(x,y)",
            marker = list(
  ...
  else if (input$hsulyplot_tipus == "hipergeo_plot_2")
            plot_ly(y=dhyper(hipergeo_n_range(),input$hipergeo_m,input$hipergeo_n,input$hipergeo_k),
                    x=hipergeo_n_range(),
                    type = "scatter",
                    name = "(x,y)",
  ...
  if (input$hipergeo_tipus == "hipergeo_intervallum")
    sulyfvg <- sulyfvg %>% add_lines(y=hipergeo_intereredmeny(),x=hipergeo_x_range(), name = "Intervallumba esés valószínűsége", 
                                     line = list(shape = "spline",color = 'FF0000'),
                                     marker = list(color = '#FF0000'),fill = "tozeroy",fillcolor = 'rgba(255, 0, 0 0.4)')
  else{
    sulyfvg <- sulyfvg %>% add_lines(y=hipergeo_1eredmeny(), x=input$hipergeo_x,name = "Valószínűség",
                                     marker = list(color = '#FF0000'))}
  sulyfvg <- sulyfvg %>% layout(xaxis = list(rangemode = "tozero",showspikes = T),
                                      yaxis = list(rangemode = "nonnegative",showspikes = T),barmode = "overlay")

  eloszlasfvg <- if (input$heloszlplot_tipus == "hipergeo_eloszlplot_1")
    plot_ly(y=phyper(hipergeo_n_range(), input$hipergeo_m, input$hipergeo_n, input$hipergeo_k),
                    x=hipergeo_n_range(),
                    type = "scatter",
            name = "(x,y)",
            mode = 'lines',line = list(color = '#009999'))
  else if (input$heloszlplot_tipus == "hipergeo_eloszlplot_2")
            
            plot_ly(x=hipergeo_n_range(),
                    y=phyper(hipergeo_n_range(), input$hipergeo_m, input$hipergeo_n,input$hipergeo_k),
                    type = "violin",
                    name = "(x,y)",
                    line = list(color = '#009999'))
  
  if (input$hipergeo_tipus == "hipergeo_intervallum")
    eloszlasfvg <- eloszlasfvg %>% add_lines(y=hipergeo_intereredmeny(),x=hipergeo_x_range(), 
                                             name = "Intervallumba esés valószínűsége", line = list(shape = "spline",color = '#FF0000'),
                                                     marker = list(color = '#FF0000'),fill = "tozeroy",fillcolor = 'rgba(255, 0, 0 0.4)')
  else{
    eloszlasfvg <- eloszlasfvg %>% add_lines(y=hipergeo_1eredmeny(),x=input$hipergeo_x, name = "Valószínűség",
                                             marker = list(color = '#FF0000'))}
...
hipergeo_n_range <- reactive({0:input$hipergeo_n})
    
hipergeo_x_range <- reactive({input$hipergeo_x_intervallum[1]:input$hipergeo_x_intervallum[2]})
    
hipergeo_intereredmeny <- reactive({(dhyper(hipergeo_x_range(),input$hipergeo_m,input$hipergeo_n,input$hipergeo_k))})   
    
hipergeo_sumintereredmeny <- reactive({format(sum(dhyper(hipergeo_x_range(),input$hipergeo_m,input$hipergeo_n,input$hipergeo_k)), digits = 5 )})

hipergeo_1eredmeny <- reactive({
        if (input$hipergeo_tipus == "hipergeo_egyenlo")
          format(dhyper(input$hipergeo_x,input$hipergeo_m,input$hipergeo_n,input$hipergeo_k), digits = 5)
  else if (input$hipergeo_tipus == "hipergeo_kisebb"){
          format(abs(phyper(input$hipergeo_x,input$hipergeo_m,input$hipergeo_n,input$hipergeo_k, lower.tail = FALSE)+dhyper(input$hipergeo_x,input$hipergeo_m,input$hipergeo_n,input$hipergeo_k)-1), digits = 5)}
  else if (input$hipergeo_tipus == "hipergeo_ekisebb"){
          format(abs(phyper(input$binom_x, input$hipergeo_m, input$hipergeo_n,input$hipergeo_k, lower = FALSE)-1), digits = 5)}
  else if (input$hipergeo_tipus == "hipergeo_nagyobb"){
      format(phyper(input$hipergeo_x, input$hipergeo_m, input$hipergeo_n,input$hipergeo_k, lower.tail = FALSE), digits = 5)}
  else if (input$hipergeo_tipus == "hipergeo_enagyobb"){
      format(phyper(input$hipergeo_x, input$hipergeo_m, input$hipergeo_n, input$hipergeo_k, lower.tail = FALSE)+dhyper(input$hipergeo_x,input$hipergeo_m,input$hipergeo_n,input$hipergeo_k),
             digits = 5)}
  })

output$hipergeo_slider_1 <- renderUI({
  sliderInput("hipergeo_k", "Adja meg a \\(k\\) értékét:", 
                    min = 0, max(input$hipergeo_n+input$hipergeo_m), value = c(5), step = 1)
  })  
    
hipergeo_df <- reactive({ 
        
        if (input$hipergeo_tipus == "hipergeo_intervallum")
            data.frame(x=hipergeo_x_range(),y=hipergeo_intereredmeny())
        
        else{
            
            data.frame(x=input$hipergeo_x,y=hipergeo_1eredmeny())}
  })  
    
output$hipergeo_tabla <- renderDataTable({
  
  hdt <- datatable(hipergeo_df(),colnames = c("X","Valószínűség"),rownames = FALSE,
                   options = list(lengthChange = FALSE,searching = FALSE,info = FALSE,
                                  language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Hungarian.json'), pageLength = 2))
  
  hdt2 <- formatStyle(hdt,"x",color = '#FFFFFF', backgroundColor = "#006666", fontWeight = 'bold')
  hdt3 <- formatStyle(hdt2,"y",color = '#FFFFFF', backgroundColor = "#009999", fontWeight = 'bold')})
    

hn <- reactive({input$hipergeo_n})  
hk <- reactive({input$hipergeo_k})
hm <- reactive({input$hipergeo_m})


hipergeo_varhato_ertek_react <- reactive ({format((hk()*hm())/(hm()+hn()), digits = 5)})

hipergeo_szoras_react <- reactive ({format((hk()*hm()*hn()*(hm()+hn()-hk()))/((hm()+hn())^2*(hm()+hn()-1)), digits= 5)})
    
output$hipergeo_box1 <- renderValueBox({
        valueBox(hipergeo_varhato_ertek_react(),icon = icon(" fa-calculator"), "Várható érték", color = "aqua", 
  href = NULL)
    })
    
output$hipergeo_box2 <- renderValueBox({  
         valueBox(hipergeo_szoras_react(), "Szórás",icon = icon(" fa-calculator"), color = "aqua",
  href = NULL)
    })
...
output$hipergeo_box3 <- renderValueBox({
        ...
        if (input$hipergeo_tipus == "hipergeo_intervallum")
            ...
            valueBox(hipergeo_sumintereredmeny(), "Intervallumba esés valószínűsége",icon = icon(" fa-calculator"), color = "aqua",
  href = NULL)
        
        else{
            
            valueBox(hipergeo_1eredmeny(), "Valószínűség",icon = icon(" fa-calculator"), color = "aqua", 
  href = NULL)
        }})
    
output$hipergeo_ex1 <- renderUI({
    withMathJax(helpText('$$X\\sim\\text{Hip}(n,K,N)$$
                          $$\\mathbf{P}(X=k)=\\frac{\\displaystyle{N-K \\choose n-k}{K\\choose k}}{\\displaystyle{N\\choose n}}$$'))
  })

output$hipergeo_ex2 <- renderUI({
    withMathJax(helpText('$$\\mathbf{E}(X)=n\\frac{K}{N}$$'))
  })

output$hipergeo_ex3 <- renderUI({
    withMathJax(helpText(' $$\\mathbf{D}^2(X)=n\\frac{K}{N}\\frac{N-K}{N}\\frac{N-n}{N-1}$$'))
  })
